...
function generatePlot01(person, planet) {
    return {
        ...
        description: `
            ...
            our hero realized that not everything is all right.
            Against own will, our hero is dragend into a rescue mission of unique organism, which is on the brink of destruction.
            Its destruction could start a chain reaction, which unimaginable consequences.  
        `
    };
}
...
function generatePlot03(person, planet, vehicle) {
    return {
        ...
        ${person} has to stop catastrophe before it destroys live on entire planet ${planet}.
        But before ${person} can try to prevent it, a ${vehicle} must be repaired in order to get there in time.
        `
    };
}
...
function generatePlot04(person, planet, species) {
    return {
        ...
        No one has seen or heard about ${species} for two thousand years. Then why now, and why here ?
        `
    };
}
...
function generatePlot05(person, planet, starship, vehicle) {
    return {
        ...
        A help is on its way. But squadron of fast cruisers got badly damaged in a sudden ion storm and precious time
        ...
        He never said a word about what he saw inside, or how he managed to get out when so many has entered the base only
        ...
        Time is running out, and only ${person} on a beloved ${vehicle} can find him in time to unlock the mysterious base
...
function generatePlot07(person, planet, vehicle, species) {
    return {
        ...
        It was supposed to be routine journey between two remote cities on a mine planet ${planet}.
        Fast sand storm has as always hit the convoy.
        In past 2 years ${person} and ${vehicle} got hit by dozens of them and lived another day.
        This one was however different and ${vehicle} broke down.
        ...
        With sand storm approaching and no way how to send a distress signal, there is only option left for ${person}.
        `
    };
}
...
function generatePlot08(person, planet, starship, vehicle, species) {
    return {
        ...
        Ten years has passed and ${person} got used to a peaceful life, well sort of peaceful life, on a planet ${planet}.
        It will be 3 years, since starting escorting convoys.
        It is an ok job, most of time it is enough if ${vehicle} looks terrifying and all wanna be robbers will think twice,
        ...
        quarter battle took another two hours before the self destruction counting started.
